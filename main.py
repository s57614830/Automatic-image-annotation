...
class MainGUI:
    def __init__(self, master):
        ...
        # csv-xml
        self.openBtn = Button(self.ctrlPanel, text=' To XML', command=self.To_xml)
        self.openBtn.pack(fill=X, side=TOP)

        self.mb.menu = Menu(self.mb, tearoff=0)
        self.mb["menu"] = self.mb.menu
        self.addCocoBtn = Button(self.ctrlPanel, text="Add", command=self.add_labels_coco)
        ...
        self.canvas = Canvas(self.frame, width=1300, height=1080)
    ...
    def To_xml(self):
        ...
        # 修改为你自己的路径
        template_file = './xml/anno.xml'
        target_dir = './annotations/XML/'
        image_dir = '/home/buu123/topic/AI+/images/'  # 图片文件夹
        train_file = 'data.txt'  # 存储了图片信息的txt文件
    ...
    def save(self):
        if self.filenameBuffer is None:
            ...
            for idx, item in enumerate(self.bboxList):
                self.annotation_file.write(self.imageList[self.cur] + ',' +
...
if __name__ == '__main__':
    root = Tk()
    imgicon = PhotoImage(file='union.png')
